...
window.__cook=function(opt){
  ...
  renderAll();
  /* Guilds get their own tab in the real thing. For the preview the skill banner
     and the Activities/Tree row are hidden so the panel reads as its own screen. */
  if(opt.guild){
    var bw=document.querySelector('.sk-banner-wrap'); if(bw) bw.style.display='none';
    var t=document.getElementById('tabActs'); if(t&&t.parentElement) t.parentElement.style.display='none';
    var xd=document.getElementById('xpDetail'); if(xd) xd.style.display='none';
    var pt=document.getElementById('panelTitle');
    if(pt) pt.innerHTML=iconHTML('ui_paw')+' Guild Hall';
...
(async()=>{
  ...
  const jobs=[{id:'now',inject:''}].concat(concepts.map(c=>{
    ...
    const sh=fs.readFileSync(path.join(__dirname,'_shared.js'),'utf8');
  ...
  for(const job of jobs){
    ...
    await p.evaluate(o=>window.__cook(o),{guild:true,fire:job.fire||'burning',view:job.view||null,

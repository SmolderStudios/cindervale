...
## How the six sheets were actually sliced (0.9.122.32) — DONE

Keep these. Every one of them needed a hand-measured crop, and re-deriving them
costs half an hour each time.

```bash
# stolen goods — 3x2, drawn table frame, captions under each cell
node _iconart/slice.js sheets/thief_loot.png  sheets/thief_loot.txt  --grid 3x2 --crop 10,10,1526,1014 --inset 8

# lockpicks + gear — 4x3 on the page, but sliced ROW BY ROW. A whole-sheet 4x3
# leaves the caption inside the content box on eight of the eleven cells (the pick
# tips reach down into the caption band, so --caption cannot separate them) and
# every lockpick shipped with "2. Iron Lockpicks" faintly printed under it.
node _iconart/slice.js sheets/thief_gear.png sheets/thief_gear_r1.txt --grid 4x1 --crop 10,10,1438,318   --inset 6
node _iconart/slice.js sheets/thief_gear.png sheets/thief_gear_r2.txt --grid 4x1 --crop 10,360,1438,676  --inset 6
node _iconart/slice.js sheets/thief_gear.png sheets/thief_gear_r3.txt --grid 4x1 --crop 10,712,1438,1036 --inset 22

# tree nodes — 4x4, no frame, three empty cells at the end
node _iconart/slice.js sheets/thief_nodes.png sheets/thief_nodes.txt --grid 4x4

# the skill icon and the cape are one object each
node _iconart/slice.js sheets/thief_skill.png sheets/thief_skill.txt --grid 1x1
node _iconart/slice.js sheets/thief_cape.png  sheets/thief_cape.txt  --grid 1x1
```

**Row 3's `--inset 22` is the one to understand.** The table's row divider sits
right at y=712, so a crop that starts there puts a full-width black rule inside the
cell; the bounding box then spans the whole cell width and the trousers come out
squashed with a line across the top. `--inset` pulls every cell in from all four
sides, which is what walks past a drawn rule. `--caption` cannot help — that only
trims a band at the BOTTOM.

Then:

```bash
node _iconart/key.js --only "<comma,separated,ids>"
node _iconart/pack.js --picks _iconart/picks.json        # the 18 item-ish ids
node _iconart/inject.js
node _iconart/pack.js --picks _iconart/picks_nodes.json --out pack_nodes.json --size 112
node _iconart/injectnodes.js --pack pack_nodes.json      # the 13 tree nodes
node _iconart/banner.js sheets/thief_banner.png thieving --band 0.17,0.49
```

The last two tools are new. `injectnodes.js` is the first thing that has ever
written NODE_ART (the original 160 went in by hand), and `banner.js` is the first
thing that has ever written SKILL_ART. Both MERGE rather than replace, because
neither block has a sheet on disk to rebuild the rest of itself from.

### Still outstanding — one cell

`th_gm_2x_xp` is the only Thieving tree node without art; it was not on the sheet.
It borrows Agility's, and `_audit_tests.js` asserts that it is the ONLY borrow, so
adding art for it and forgetting to inject fails loudly.

````
SHEET: One skill-tree node icon for a dark-fantasy RPG. A single cell, one object.

Match the thieving node sheet: hand-painted, dark ink outline, cel shading in three
or four value steps, flat pure white background, the object drawn large and filling
the frame. Cold steel-blue and near-black with one warm accent.

  Fast Talker — an open hand raised palm-out mid-gesture, the way someone talks
                their way past a guard. Not a weapon, not a coin. One hand, no face.
````
